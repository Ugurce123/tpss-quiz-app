...
router.get('/start/:levelId', authenticateToken, requireApproval, async (req, res) => {
  try {
    ...
    if (!level) {
      console.log('❌ Seviye bulunamadı:', req.params.levelId);
      return res.status(404).json({ 
        message: 'Seviye bulunamadı' 
      });
    }

    console.log('✅ Seviye bulundu:', level.name, 'Level:', level.level);
    ...
    console.log('📊 Sorular bulundu:', {
      count: questions.length,
      withImages: questions.filter(q => q.image).length,
      firstQuestionImage: questions[0]?.image
    });
